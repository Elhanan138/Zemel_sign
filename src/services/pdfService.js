...
async function embedSignaturesAndFinalize(documentId) {
  ...
  const sourcePath = path.join(__dirname, '../../uploads', doc.stored_name);
  ...
  // Add audit stamp on last page
  const lastPage = pages[pages.length - 1];
  const { width: lw, height: lh } = lastPage.getSize();
  const allSigners = db.prepare('SELECT * FROM signers WHERE document_id = ?').all(documentId);
  const stampLines = [
    `Document signed via Zemel Sign`,
    `Document: ${doc.title}`,
    ...allSigners.map(s => `Signer: ${s.name} <${s.email}> - ${s.signed_at || 'pending'}`),
  ...
  const outPath = path.join(__dirname, '../../processed', `${doc.stored_name}_signed.pdf`);
